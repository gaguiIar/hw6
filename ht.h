...
{
    HASH_INDEX_T cap = CAPACITIES[mIndex_];
    if (static_cast<double>(numOccupied_ + 1) / cap >= resizeAlpha_)
    {
        resize();
        cap = CAPACITIES[mIndex_];
    }
    ...
    table_[loc]->item.second = p.second; // key already exists
}
...
{
    const size_t capCount = sizeof(CAPACITIES) / sizeof(CAPACITIES[0]);
    if (static_cast<size_t>(mIndex_ + 1) >= capCount)
    {
    ...
    table_.assign(newCap, nullptr);
    num_items_ = 0;
    numOccupied_ = 0;

    // Rehash all items alr in table
    for (HashItem *node : oldTable)
    {
        if (node == nullptr)
            continue;
        ...
        // Reinsert node into new table using probing
        HASH_INDEX_T start = hash_(node->item.first) % newCap;
        prober_.init(start, newCap, node->item.first);
